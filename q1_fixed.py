...
class NetworkSliceSystem:
    ...
    def process_task(self, task, allocated_rb, user_data):
        ...
        # 计算本次时间片可传输的数据量（更合理的传输速度）
        data_per_slot = rate * self.time_slot / 1000  # Mbit
    ...
    def evaluate_qos(self, task, rate, delay):
        ...
        if task.slice_type == 'URLLC':
            ...
            if rate >= self.R_e_min:
                ...
                return max(0.0, rate / self.R_e_min)  # 确保不为负数
    ...
    def process_slice_tasks(self, slice_type, user_data):
        ...
        for task in queue:
            ...
            if completed:
                ...
                # 只有在延迟超时或速率严重不足时才标记为失败
                if (task.slice_type == 'URLLC' and delay > self.D_u_max) or \
                   (task.slice_type == 'eMBB' and (delay > self.D_e_max or rate < 1)) or \
                   (task.slice_type == 'mMTC' and delay > self.D_m_max):
                    task.is_failed = True
    ...
    def run_simulation(self, user_data, simulation_time=50):
        """运行仿真"""
        print("=== 网络切片系统仿真（修正版）===")
...
def solve_problem_1_fixed():
    """使用修正版本解决第一题"""
    system = NetworkSliceSystem()
    user_data = system.load_data('data1.xlsx')
    
    # 运行仿真
    best_allocation, total_qos = system.run_simulation(user_data, simulation_time=50)
