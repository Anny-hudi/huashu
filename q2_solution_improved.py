...
def solve_problem_2_improved():
    """解决第二题：改进版本，修复QoS计算问题"""
    ...
    def optimize_resource_allocation(user_data, queue_tasks):
        """优化资源分配"""
        best_qos = -float('inf')
                            ...
                            if mmtc_constraint_ok:
                                ...
                                if urllc_rbs > 0:
                                    for user in user_mapping['URLLC']:
                                        if user in user_data and user_data[user]['task_size'] > 0:
                                            ...
                                            num_rbs = urllc_rb_per_user_actual
                                            
                                            # 计算SINR和速率
                                            sinr = calculate_sinr(power, user_info.get('large_scale', 0), 
                                                                 user_info.get('small_scale', 1), 
                                                                 user_info['x'], user_info['y'], num_rbs)
                                            rate = calculate_rate(sinr, num_rbs)
                                ...
                                if embb_rbs > 0:
                                    for user in user_mapping['eMBB']:
                                        if user in user_data and user_data[user]['task_size'] > 0:
                                            user_info = user_data[user]
                                            num_rbs = embb_rb_per_user_actual
                                ...
                                if mmtc_rbs > 0:
                                    ...
                                    for user in user_mapping['mMTC']:
                                        if user in user_data and user_data[user]['task_size'] > 0:
                                            user_info = user_data[user]
                                            num_rbs = mmtc_rb_per_user_actual
    ...
    for decision_idx in range(num_decisions):
        ...
        # 优化资源分配
        best_allocation, best_qos = optimize_resource_allocation(user_data, task_queue)
